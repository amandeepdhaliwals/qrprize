...
class StoresController extends BackendBaseController
{
        ...
        return Datatables::of($$module_name)
            ...
            ->editColumn("step_completed", function ($data) {
                ...
                if ($data->step_completed >= 1) {
                    $step1Badge =
                        '<span class="badge bg-success">Step 1 Completed</span>';
                } else {
                    $step1Badge =
                        '<span class="badge bg-warning">Step 1 Pending</span>';
                }

                // Step 2 badge
                if ($data->step_completed >= 2) {
                    $step2Badge =
                        '<span class="badge bg-success">Step 2 Completed</span>';
                } else {
                    $step2Badge =
                        '<span class="badge bg-warning">Step 2 Pending</span>';
                }

                // All steps badge
                if ($data->step_completed == 3) {
                    $allStepsBadge =
                        '<span class="badge bg-success">All Steps Completed</span>';
                } elseif (
                    $data->step_completed >= 1 &&
                    $data->step_completed < 3
                ) {
                    $allStepsBadge =
                        '<span class="badge bg-warning">Qr Code Pending</span>';
                }
    ...
    public function store(Request $request)
    {
        ...
        $fields = ['address', 'bio', 'url_website', 'url_facebook', 'url_instagram', 'url_twitter', 'url_linkedin', 'avatar'];
        $allEmpty = true;
        ...
        $$module_name_singular_user = User::findOrFail($ $$module_name_singular->id);
        if ($userprofile) {
    ...
    public function storeCampaign(Request $request)
    {
        ...
        $$module_name_singular = $module_model::create([
            ...
            "user_id" => $storeId,
            "total_no_of_coupons" => "0",
            "no_of_winned_coupons" => "0",
            "qr_code_url" => $url,
