...
<x-backend.layouts.show :data="$user">
    ...
    <x-backend.section-header>
        ...
        <x-slot name="toolbar">
            ...
            <x-buttons.edit route='{!!route("backend.stores.edit", $$module_name_singular)!!}' title="{{__('Edit')}} {{ ucwords(Str::singular($module_name)) }}" />
            @endcan
        </x-slot>
    </x-backend.section-header>
        ...
        <div class="col">
                ...
                <table class="table table-hover">
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            @auth
                            @if ($user->email_verified_at == null&& auth()->user()->hasRole('super admin'))
            ...
            <div class="py-4 text-center">
               @can('delete_'.$module_name)
                ...
                @endif
                <a href="{{route('backend.stores.destroy', $user)}}" class="btn btn-danger mt-1" data-method="DELETE" data-token="{{csrf_token()}}" data-toggle="tooltip" title="{{__('labels.backend.delete')}}" data-confirm="Are you sure?"><i class="fas fa-trash-alt"></i> Delete</a>
                @endcan
                @auth
                @if ($user->email_verified_at == null&& auth()->user()->hasRole('super admin'))
                    <a href="{{ route('backend.users.emailConfirmationResend', auth()->user()->id) }}" class="btn btn-primary mt-1" data-toggle="tooltip" title="Send Confirmation Email">
